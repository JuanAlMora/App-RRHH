...
router.post('/editarCandidato', (req, res) => {
    if (req.body.id) {
        id = req.body.id;
    }else{
        ...
        res.redirect('/detallesCandidato');
    } 
})    

router.get('/editarCandidato', (req, res) => {
    connection.query('SELECT id, nombre, correo, direccion, documento, telefono, Nomina_id, copia_cedula, cv, libreta_militar, foto, certificado_pension, certificado_eps FROM recurso_humano WHERE id = ?', [id], (err, row) => {
        ...
        res.render('recursoHumano/candidatos/editPerfilCandidato.hbs', {row})
    })  
})
